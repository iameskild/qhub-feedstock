...
Once the `conda-forge` channel has been enabled, `qhub` can be installed with `conda`:
...
It is possible to list all of the versions of `qhub` available on your platform with `conda`:
...
```
mamba search qhub --channel conda-forge
```
...
mamba repoquery search qhub --channel conda-forge

# List packages depending on `qhub`:
mamba repoquery whoneeds qhub --channel conda-forge

# List dependencies of `qhub`:
mamba repoquery depends qhub --channel conda-forge
```
